...
File attributes include SDR type, centre frequency, sample rate, FFT size, gain, and creation timestamp. When launched from the scheduler, additional observation metadata is stored: observation name, coordinate system, target coordinates, calibrator state, scheduled start time, and duration. Spectra are stored in linear power units (not dB) to preserve radiometric accuracy for calibration workflows.
