...
Spectra are stored in linear power units (not dB) to preserve radiometric accuracy for calibration workflows.
...
\midrule
\texttt{/status} & Current position (alt, az, target\_alt, target\_az), motor currents, status string, fault, is\_slewing, calibrator state \\
\texttt{/tracking} & Tracking state: enabled, RA/Dec, target name, offsets, waiting flags \\
