...
| Script | Purpose |
|---|---|
| `astro_simulator.py` | Interactive GUI: click the all-sky map, get the dish spectrum. Editable pointing, beam, T_sys, integration time, bandwidth, band centre, spectrometer channel count (pre-filled with the band's native count at startup; emptying the box also means native resolution; the spectrum draws as channel steps) and velocity frame (LSR/SSB/topocentric); target list (H I targets plus the continuum sources Cyg A, Cas A, Tau A, the Sun and the Moon), a map toggle between the N_HI and 1420 MHz continuum skies, site horizon/visibility overlays, and a live RA/Dec + Alt/Az readout for the current pointing beside the map. The lower panel is modal: with the H I map it shows the spectrum; with the continuum map it shows a **drift scan** — band-averaged T_A (H I included when in-band) for the sky drifting through a beam parked on the current target, any target, above the horizon or not, centred on beam-centre transit over the duration set in the **scan (min)** box, with per-sample noise. A **Realise** button hands the simulation to the real telescope (ESP32 controller, `--controller` URL): with the H I map it starts galactic tracking of the pointing; with the continuum map it cancels tracking and parks the dish at the drift-scan start, so the target transits the beam centre half a scan later. Press `s` to save the current spectrum (PNG + txt). |
