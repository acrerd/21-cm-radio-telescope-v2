                ...
                <div class="left-col">
                    ...
                    <div class="box">
                        ...
                        <div class="axis-switch" id="axis_switch">
                            ...
                            <button class="axis-option active fault-dependent" id="axis_both" onclick="setAxisMode('both')" data-help="Track both altitude and azimuth for the selected target.">Tracking in both directions</button>
                        ...
                        <div class="axis-fixed-row" id="axis_fixed_row">
                            ...
                            <span class="target-info" id="axis_fixed_help" style="margin-top:0;"></span>
                        </div>
                        <div class="action-grid">
                            <span class="target-info" style="margin-top:0;">Calibrator:</span>
                            <div class="axis-switch cal-switch" id="cal_switch">
                                <button class="axis-option active fault-dependent" id="cal_off" onclick="setCalibrator(false)" data-help="Turn the calibrator noise source off.">Off</button>
                                <button class="axis-option fault-dependent" id="cal_on" onclick="setCalibrator(true)" data-help="Turn the calibrator noise source on for receiver calibration.">On</button>
                            </div>
...
function scanWifi(){document.getElementById('wifi-networks').innerHTML='<p style="color:#888;">Scanning...</p>';fetch('/wifi/scan').then(r=>r.json()).then(networks=>{if(networks.length===0){document.getElementById('wifi-networks').innerHTML='<p style="color:#888;">No networks found</p>';return;}let html='';networks.forEach(n=>{const sig=n.rssi>-50?'####':n.rssi>-60?'###-':n.rssi>-70?'##--':'#---';const sec=n.secure?'[+]':'';html+='<div class="wifi-network" data-help="Select this WiFi network and open the password form." onclick="selectNetwork(\''+n.ssid+'\')">'+sec+' '+n.ssid+'<span class="wifi-signal">'+sig+'</span></div>';});document.getElementById('wifi-networks').innerHTML=html;});}
...
function updateFixedAxisRow(d){const row=document.getElementById('axis_fixed_row');const label=document.getElementById('axis_fixed_label');const input=document.getElementById('axis_fixed_value');const help=document.getElementById('axis_fixed_help');if(!row||!label||!input||!help)return;const editing=document.activeElement===input;if(d.az_only){row.classList.add('active');label.textContent='Fixed altitude';input.min='0';input.max='90';if(!editing)input.value=Number(d.az_only_alt||0).toFixed(1);input.setAttribute('data-help','Altitude to hold while the telescope tracks only azimuth.');help.textContent='Azimuth keeps tracking; altitude stays at this value.';}else if(d.alt_only){row.classList.add('active');label.textContent='Fixed azimuth';input.min='0';input.max='360';if(!editing)input.value=Number(d.alt_only_az||0).toFixed(1);input.setAttribute('data-help','Azimuth to hold while the telescope tracks only altitude.');help.textContent='Altitude keeps tracking; azimuth stays at this value.';}else{row.classList.remove('active');help.textContent='';}}
