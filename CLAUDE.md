...
**Obstructed horizon**: `obstruction_sectors` in `scheduler_config.json` is a list of `[az_min, az_max, min_sun_alt]` (default `[[45, 120, 30]]`, the eastern treeline). The calibration day does not scan the Sun while it is inside a sector, and `fit_pointing_model(obstruction_sectors=...)` leaves any scan already on file from inside one out of the fit, reporting the count as `n_obstructed`. The reason is that trees are a **source** at 1420 MHz, not a screen: foliage at ~290 K entering the lower rows of the raster puts a ramp under the Sun and drags the single Gaussian's centroid down into it, so the scan is wrong rather than merely noisy and its stated centroid uncertainty gives no warning. On 2026-08-20 the first four scans (Sun 18–29°, az 94–112°) fitted 0.5–1.2° low in altitude while the evening scans at the *same altitudes* in the west were clean — that east/west asymmetry, not the low elevation, is what identifies it. The altitude in a sector is a floor for the **Sun**, already including the raster and beam half-widths, so it is read off a calibration day rather than surveyed. Like `true_lat`/`true_lon`, the sectors are passed into the fit by the caller so the fit stays a pure function of the data it is handed.

**Horizon scan** (`receiver_scheduler/horizon_scan.py`): maps the obstructed horizon radiometrically — trees, roofline and the dome tower are ~290 K sources at 1420 MHz, so lowering the beam through them is a large step in total power. One altitude cut per azimuth at uniform 5°, fitted with a beam-convolved step `P = P_sky + C/2·erfc((alt−edge)/(σ√2))`. The **50% crossing is the geometric edge** and is independent of beam width (which the same fit measures, as a check); the **clearance altitude** is read off the skirt of that same curve at `clearance_fraction` (1%) of the step, not assumed as a multiple of the beam. Store the geometric edge and the clearance, never an activity-specific floor — baking in a Sun raster's half-extent is what made `obstruction_sectors` unreusable. Raw cuts are stored with every record for the same reason the version-1 sun scans are dead weight: conclusions can't be reprocessed. Cuts are **serpentine** (alternate direction, no return slew), so backlash lands as an azimuth-parity zigzag — `direction_bias()` tests for exactly that and it is shown in the UI. Where the step model rejects itself (double step, wrong width, large residuals) the estimator falls back to a conservative **envelope**: the highest altitude still showing ground, which assumes nothing about the shape. Between sampled azimuths `horizon_floor()` takes the **worse** of the two neighbours, since a narrow obstruction is likelier to be missed than double-counted. `--sdr demo` runs the whole scan against a synthetic horizon with no hardware, and a demo-sourced profile is flagged in the UI so it cannot be mistaken for the real one. Run it at night, dry and calm; re-measure seasonally, since trees grow and leaf-on/leaf-off are different horizons.
...
**Slew polling**: `_slew_to` polls `/status` every `_SLEW_POLL_INTERVAL_S` (0.1 s), not the 1 s it used to. Measured 2026-08-21: axes run at 3.9°/s in altitude and 2.9°/s in azimuth, a 1° move costs ~1.5 s end to end of which only 0.3 s is motion, so the old interval was most of the cost of a raster. Do **not** poll flat out — with no gap at all the ESP32 resets the connection after a few requests; 50 ms sustained 14 Hz in testing and 100 ms keeps margin. Faster polling means ~10× more requests per slew, so transient status failures are now tolerated up to `_SLEW_STATUS_FAILURES_ALLOWED` before a slew is abandoned. Structural wobble was measured (camera, sub-pixel) at ≲0.2–0.3° after a slew and was judged too small to need a settle pause; the encoders see nothing at all, since the flex is beyond them.
