...
**Code review status**: `docs/code_review_2026-08-05.md` records a full review of the controller and scheduler with per-finding status notes. All high-severity scheduler/sun-scan items are fixed; every ESP32 firmware finding (C1-C15, chiefly the missing cross-task locking) is still open and needs a bench ESP32. `tools/due_emulator.py` emulates the Due serial protocol over a USB-UART adapter for exactly that bench work (wiring instructions in its docstring; requires pyserial). Before fixing C7 (`/goto` silently enables tracking), check which callers rely on goto-implies-tracking.

**Scheduler/receiver tests**: run with the radioconda Python: `python -m pytest receiver_scheduler/test_scheduler.py receiver_scheduler/test_sun_scan.py` (125 tests, all green as of 2026-08-05). The receiver GUI runs without hardware via `--sdr demo`; two receiver instances conflict over the default `h1_data.h5` file lock.
